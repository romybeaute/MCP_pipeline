...
def register():
    for f in get_avaialable_functions():
        register_module('reductions', 'permed/rs/bv62/{}'.format(f),
                        _get_rs_bv62)
        register_module('reductions', 'permed/rs/bv62gfp/{}'.format(f),
                        _get_rs_bv62gfp)

def _get_rs_bv62gfp(config, config_params):
    if len(config) == 0:
        config == 'mean'
    epochs_fun = get_function_by_name(config)
    channels_fun = np.std
    return _get_rs_bv62_generic(epochs_fun, channels_fun)


def _get_rs_bv62(config, config_params):
    if len(config) == 0:
        config == 'mean'
    epochs_fun = get_function_by_name(config)
...
def _get_rs_bv62_generic(epochs_fun, channels_fun):
    return _get_rs_montage_generic(
        montage='permed/bv62', epochs_fun=epochs_fun, channels_fun=channels_fun)
