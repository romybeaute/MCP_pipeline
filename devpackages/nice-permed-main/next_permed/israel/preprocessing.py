...
from nice_ext.equipments import _bs_filter
from nice_ext.algorithms.adaptive import _adaptive_egi, find_bad_components

# from .constants import _biosemi64_loc_names
from ..equipments import _permed_ch_names


def register():
    register_module('preprocess', 'permed/rs/raw/biosemi', _preprocess_rs_raw_biosemi)
    # register_module('preprocess', 'icm/rs/raw/egi', _preprocess_rs_raw_egi)
    


def _preprocess_rs_raw_biosemi(raw, config_params):
    ...
    # n_eeg = len(mne.pick_types(raw.info, eeg=True)) # number of channels
    # good_channels = ['E{}'.format(x) for x in range(1, n_eeg + 1)]  # TODO to change
    good_channels = [x for x in _permed_ch_names['bs64']] # TODO is this working?
